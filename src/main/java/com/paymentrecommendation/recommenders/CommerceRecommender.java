...
public class CommerceRecommender extends RecommendationService implements PaymentRecommender {
    ...
    private List<PaymentInstrument> getPaymentInstruments(List<PaymentInstrument> userPaymentInstruments, Cart cart, boolean upiEnabled){
        ...
        List<PaymentInstrumentType> commercePaymentInstruments = CommerceRelevance.commercePaymentInstrumentRelevance;

        HashMap<PaymentInstrumentType, List<PaymentInstrument>> eligiblePaymentCategories = Utils.getEligiblePaymentCategories(
                userPaymentInstruments, cart.getCartDetail().getCartAmount(), commercePaymentInstruments, new CommerceRelevance());

        for(PaymentInstrumentType paymentInstrumentType : commercePaymentInstruments){
